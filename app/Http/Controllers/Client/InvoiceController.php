   ...
   public function index()
{
    ...
    $baseQuery = \App\Models\Invoice::where('client_id', $user->id)
        ->whereNull('deleted_at');

    // KPI totals
    $totalBilled      = (clone $baseQuery)->sum('total');
    $totalPaid        = (clone $baseQuery)->sum('amount_paid');
    $totalOutstanding = (clone $baseQuery)->whereIn('status', ['unpaid', 'partial', 'overdue'])->sum('balance');

    // Tab counts
    $tabCounts = [
        'all'     => (clone $baseQuery)->count(),
        'unpaid'  => (clone $baseQuery)->where('status', 'unpaid')->count(),
