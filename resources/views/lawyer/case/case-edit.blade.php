...
@section('page_subtitle', 'Update case information for ' . $case->title)

@section('topbar_actions')
<a href="{{ route('lawyer.cases.show', $case) }}" class="btn-secondary">
...
<form action="{{ route('lawyer.cases.update', $case) }}" method="POST">
    @csrf
    @method('PUT')
        ...
        <div class="cf-card-body">
            <div class="cf-grid">
                <div class="cf-field">
                    ...
                    <input type="text" id="title" name="title" class="cf-input" placeholder="Enter case title" value="{{ old('title', $case->title) }}" required>
                ...
                <div class="cf-field">
                    ...
                    <select id="case_category_id" name="case_category_id" class="cf-input cf-select" required>
                        ...
                        <option value="{{ $cat->id }}" {{ old('case_category_id', $case->category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="cf-field">
                    ...
                    <select id="court_type_id" name="court_type_id" class="cf-input cf-select" required>
                        ...
                        <option value="{{ $ct->id }}" {{ old('court_type_id', $case->court_type_id) == $ct->id ? 'selected' : '' }}>{{ $ct->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="cf-field">
                    ...
                    <select id="client_id" name="client_id" class="cf-input cf-select" required>
                        ...
                        <option value="{{ $cl->id }}" {{ old('client_id', $case->client_id) == $cl->id ? 'selected' : '' }}>{{ $cl->name }} ({{ $cl->email }})</option>
                        @endforeach
                    </select>
                ...
                <div class="cf-field">
                    ...
                    <select id="status" name="status" class="cf-input cf-select">
                        <option value="intake"               {{ old('status', $case->status) == 'intake'               ? 'selected' : '' }}>Intake</option>
                        <option value="barangay_mediation"   {{ old('status', $case->status) == 'barangay_mediation'   ? 'selected' : '' }}>Barangay Mediation</option>
                        <option value="escalation_to_court"  {{ old('status', $case->status) == 'escalation_to_court'  ? 'selected' : '' }}>Escalation to Court</option>
                        <option value="active_case"          {{ old('status', $case->status) == 'active_case'          ? 'selected' : '' }}>Active Case</option>
                        <option value="resolution"           {{ old('status', $case->status) == 'resolution'           ? 'selected' : '' }}>Resolution</option>
                    </select>
                ...
                <div class="cf-field cf-field-full">
                    ...
                    <textarea id="description" name="description" class="cf-input cf-textarea" placeholder="Detailed description of the case" rows="4">{{ old('description', $case->description) }}</textarea>
        ...
        <div class="cf-card-body">
            <div class="cf-grid">
                <div class="cf-field">
                    ...
                    <input type="date" id="filing_date" name="filing_date" class="cf-input" value="{{ old('filing_date', $case->filed_date?->format('Y-m-d')) }}">
                    @error('filing_date')<span class="cf-error">{{ $message }}</span>@enderror
                </div>
                <div class="cf-field">
                    <label class="cf-label" for="hearing_date">Hearing Date</label>
                    <input type="date" id="hearing_date" name="hearing_date" class="cf-input" value="{{ old('hearing_date', $case->hearing_date?->format('Y-m-d')) }}">
        ...
        <div class="cf-card-body">
            <div class="cf-grid">
                <div class="cf-field">
                    ...
                    <input type="text" id="court_name" name="court_name" class="cf-input" placeholder="e.g. Regional Trial Court" value="{{ old('court_name', $case->court_name) }}">
                    @error('court_name')<span class="cf-error">{{ $message }}</span>@enderror
                </div>
                <div class="cf-field">
                    <label class="cf-label" for="judge_name">Assigned Judge</label>
                    <input type="text" id="judge_name" name="judge_name" class="cf-input" placeholder="Judge name (if assigned)" value="{{ old('judge_name', $case->judge_name) }}">
                    @error('judge_name')<span class="cf-error">{{ $message }}</span>@enderror
                </div>
                <div class="cf-field">
                    <label class="cf-label" for="opposing_party">Opposing Party</label>
                    <input type="text" id="opposing_party" name="opposing_party" class="cf-input" placeholder="Name of opposing party" value="{{ old('opposing_party', $case->opposing_party) }}">
                ...
                <div class="cf-field">
                    ...
                    <input type="text" id="opposing_counsel" name="opposing_counsel" class="cf-input" placeholder="Opposing counsel name" value="{{ old('opposing_counsel', $case->opposing_counsel) }}">
                    @error('opposing_counsel')<span class="cf-error">{{ $message }}</span>@enderror
                </div>
                <div class="cf-field cf-field-full">
                    <label class="cf-label" for="notes">Additional Notes</label>
                    <textarea id="notes" name="notes" class="cf-input cf-textarea" placeholder="Any additional notes or context" rows="3">{{ old('notes', $case->notes) }}</textarea>
