...
<body>
    <div class="container">
        <div class="card">
            <div class="header">
                <h1>LegalCase — Case Status Updated</h1>
            </div>
            <div class="body">
                <h2>Your case status has changed</h2>
                <p>Your case <strong>{{ $case->case_number }}</strong> has been updated.</p>
                <div class="details">
                    <p><strong>Case Number</strong><br>{{ $case->case_number }}</p>
                    <p><strong>Previous Status</strong><br>{{ ucfirst($oldStatus) }}</p>
                    <p><strong>New Status</strong><br>{{ ucfirst($newStatus) }}</p>
                    <p><strong>Lawyer</strong><br>{{ optional($case->lawyer)->name ?? 'Your lawyer' }}</p>
                </div>
                <p>Please review the update and contact your lawyer if you have any questions.</p>
                <a href="{{ route('client.cases.show', $case) }}" class="button">View Case</a>
            </div>
            <div class="footer">
