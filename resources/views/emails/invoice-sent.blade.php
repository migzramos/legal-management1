...
<body>
    <div class="container">
        <div class="card">
            <div class="header">
                <h1>LegalCase — New Invoice</h1>
            </div>
            <div class="body">
                <h2>Invoice #{{ $invoice->invoice_number }} has been issued</h2>
                <p>Your lawyer has sent you a new invoice for professional services.</p>
                <div class="details">
                    <p><strong>Invoice Number</strong><br>{{ $invoice->invoice_number }}</p>
                    <p><strong>Amount Due</strong><br>{{ money_display($invoice->total) }}</p>
                    <p><strong>Due Date</strong><br>{{ optional($invoice->due_date)->format('F j, Y') }}</p>
                </div>
                <p>Please review the invoice and pay before the due date to keep your case moving smoothly.</p>
                <a href="{{ route('client.invoices.show', $invoice) }}" class="button">View Invoice</a>
            </div>
            <div class="footer">
