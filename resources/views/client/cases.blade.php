...
<div class="tabs" style="margin-bottom:20px;">
    @foreach(['all'=>'All Cases','open'=>'Open','ongoing'=>'Ongoing','closed'=>'Closed'] as $key=>$label)
    <a href="{{ route('client.cases.index',['status'=>$key==='all'?null:$key]) }}" class="tab {{ ($activeTab??'all')===$key?'active':'' }}">
...
<div style="display:flex;flex-direction:column;gap:14px;">
    ...
    <div class="card">
        <div class="card-body">
            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:14px;">
                <div style="min-width:0;">
                    ...
                    <div style="font-size:0.75rem;color:var(--text-muted);">
                        Ref: {{ $case->case_number ?? 'N/A' }} &nbsp;·&nbsp; Lawyer: {{ $case->lawyer->name ?? '—' }} &nbsp;·&nbsp; Opened: {{ \Carbon\Carbon::parse($case->created_at)->format('M d, Y') }}
                    </div>
                </div>
                <div style="display:flex;align-items:center;gap:8px;flex-shrink:0;">
                    <span class="status-{{ strtolower($case->status) }}">{{ ucfirst($case->status) }}</span>
            ...
            @if($case->description)
            <p style="font-size:0.82rem;color:var(--text-secondary);line-height:1.6;margin-bottom:14px;">{{ Str::limit($case->description,180) }}</p>
            @endif
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;padding-top:14px;border-top:1px solid var(--border);">
                ...
                <div style="text-align:center;">
                    <div style="font-family:'Cormorant Garamond',serif;font-size:1.3rem;font-weight:600;color:var(--text-primary);">{{ $case->appointments_count ?? 0 }}</div>
